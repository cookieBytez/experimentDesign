...
import sys
sys.path.append('/content/drive/My Drive/ColabModels/')
...
# Define dynamic filenames based on dataset name
autoencoder_model_filename = f'{name}_autoencoder.h5'
encoder_model_filename = f'{name}_encoder.h5'

es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=10)
mc = ModelCheckpoint(autoencoder_model_filename, monitor='val_loss', mode='min', verbose=1, save_best_only=True)

model.fit(tf_train, validation_data=tf_valid, epochs=epochs, batch_size=batch_size, callbacks=[es, mc])

saved_model = load_model(autoencoder_model_filename)
...
# Define dynamic filename for saving the RNN model
rnn_model_filename = f'{name}_rnn.h5'
es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=5)
mc = ModelCheckpoint(rnn_model_filename, monitor='val_loss', mode='min', verbose=1, save_best_only=True)

history = model.fit(train_x, train_y, validation_data=(valid_x, valid_y), epochs=epochs, batch_size=batch_size, callbacks=[es, mc])

saved_model = load_model(rnn_model_filename)
