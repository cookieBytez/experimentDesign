...
encoder_model = load_model(f'{name}_autoencoder.h5')
...
''' Predict. '''
model = load_model(f'{name}_encoder.h5')
...
statistical_significans.to_csv(f'{name}_statistical_significans_auto.csv', index=False)
...
varying_thresholds.to_csv(f'{name}_varying_thresholds_auto.csv', index=False)
